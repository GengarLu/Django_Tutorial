...
def paginationProjects(request, projects, curr_results):

    current_page = request.GET.get('page') # 要顯示的頁數
    results = curr_results # 一頁顯示的資料數
    paginator = Paginator(projects, results) # results放進來
...
def searchProjects(request):
    ...
    projects = Project.objects.distinct().filter(
        ...
        # Project表格中的owner和Profile表格有關聯，所以可以透過owner__name抓到Profile的name
        Q(owner__name__icontains = search_query) |
         # Project表格中的tags和Tag表格有關聯，所以可以透過tags__in抓到Project的tags
        Q(tags__in = tags)
    )
